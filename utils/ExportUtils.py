...
class ExportUtils:
    ...
    @classmethod
    def export_csv(cls, dataset_path, session):
        file_path = cls.download_data_if_not_exists(dataset_path, session)
        file_path = file_path.split('/')[-1].split('.')[0]
        return file_path

    @classmethod
    def download_data_if_not_exists(cls, dataset_path, session):
    ...
    @classmethod
    def export_shp_file(cls, dataset_path, session):
